...
data ReducedBase = RIndex Integer 
                 | RDur  Integer 
                 | RLetter (P.Letter,P.Sign)
                 | ROctave Integer
                 | RColor [Integer] 
                 | RScale [(P.Letter,P.Sign)]
                 | RPlacement ((P.Letter,P.Sign),Integer)
                 | RNote (((P.Letter,P.Sign),Integer),Integer)
  deriving (Eq,Show)
...
reduceWith wt (FlatList (ct,ts1)) (FlatList (_,ts2)) =
...
buildScale :: (P.Letter,P.Sign) -> [Integer] -> [(P.Letter,P.Sign)]
buildScale letter color = buildScaleHelper letter (0:color)

buildScaleHelper :: (P.Letter,P.Sign) -> [Integer] -> [(P.Letter,P.Sign)]
buildScaleHelper letter (0:rest) = letter : (buildScaleHelper letter rest)
buildScaleHelper letter (n:rest) = let next_letter = nextLetter letter
                                   in buildScaleHelper next_letter (n-1:rest)
buildScaleHelper _ [] = []
